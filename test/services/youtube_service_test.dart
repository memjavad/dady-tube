...
void main() {
  ...
  group('YoutubeService.getChannelInfoById Error Path', () {
    test(
      'Returns null when YoutubeExplode fails and fallback scraping HTTP request throws exception',
      () async {
        final channelId = 'UC_ERROR_TEST';
        when(
          () => mockChannelClient.get(any()),
        ).thenThrow(Exception('YoutubeExplode failed'));
        final mockHttpClient = MockHttpClient();
        when(
          () => mockHttpClient.get(any()),
        ).thenThrow(Exception('HTTP Client failed'));
