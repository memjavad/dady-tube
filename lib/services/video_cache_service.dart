...
class VideoCacheService {
  ...
  Future<void> _manageCacheSize() async {
    ...
    // ⚡ Bolt: Use a Schwartzian transform to pre-cache lastModifiedSync()
    // This avoids severe O(N log N) blocking disk I/O during sorting
    final fileStats = files
    ...
    for (int i = 0; i < fileStats.length - _maxCacheEntries; i++) {
      try {
        final fileToDelete = fileStats[i].file;
