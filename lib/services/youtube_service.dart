...
class YoutubeService {
  ...
  static Future<YoutubeChannel?> getChannelInfoById(String id, {yt.YoutubeExplode? ytClient}) async {
    final ytExplode = ytClient ?? yt.YoutubeExplode();
    try {
  ...
  static YoutubeChannel? _parseChannelResponse(http.Response response) {
    if (response.statusCode == 200) {
      ...
      if (body.contains('ytInitialData = ')) {
        try {
          ...
          if (metadata != null) {
            ...
            if (id != 'unknown') {
              return YoutubeChannel(id: id, name: name ?? '', thumbnailUrl: thumb);
            }
          }
        } catch (e) {
      ...
      if (body.contains('<title>')) {
        channelName = body.split('<title>')[1].split('</title>')[0].replaceAll(' - YouTube', '');
      }
      ...
      if (body.contains('channelId":"')) {
        ...
        channelId = body.split('itemprop="channelId" content="')[1].split('"')[0];
      ...
      if (body.contains('"avatar":{"thumbnails":[{"url":"')) {
        thumbUrl = body.split('"avatar":{"thumbnails":[{"url":"')[1].split('"')[0];
      }

      if (channelId != 'unknown') {
        return YoutubeChannel(id: channelId, name: channelName, thumbnailUrl: thumbUrl);
      }
    }
    return null;
  }

  static Future<List<YoutubeVideo>> fetchVideosForChannel(String channelId) async {
    ...
    try {
      final url = 'https://www.youtube.com/feeds/videos.xml?channel_id=$channelId&hl=ar&gl=IQ';
      final response = await http.get(Uri.parse(url));
      ...
      if (response.statusCode == 200) {
        ...
        final videos = entries.map((entry) {
          ...
          final thumbnailUrl = mediaGroup.findElements('media:thumbnail').first.getAttribute('url') ?? '';
          ...
          return YoutubeVideo(
            ...
            channelId: channelId,
            publishedAt: DateTime.tryParse(entry.findElements('published').first.innerText) ?? DateTime.now(),
          );
        }).toList();
  ...
  static Future<List<YoutubeVideo>> _fetchVideosViaScraping(String channelId) async {
  ...
  static List<YoutubeVideo> _parseVideosJson(_ParseParams params) {
    ...
    try {
      ...
      for (var tab in tabs) {
        ...
        if (tabRenderer != null && (tabRenderer['title'] == 'Videos' || tabRenderer['title'] == 'فيديوهات' || tabRenderer['selected'] == true)) {
      ...
      final gridItems = videosTabContent['richGridRenderer']?['contents'] ??
      ...
      for (var item in gridItems) {
        final videoRenderer = item['richItemRenderer']?['content']?['videoRenderer'] ?? item['gridVideoRenderer'];
        if (videoRenderer == null) continue;

        final videoId = videoRenderer['videoId'];
        final title = videoRenderer['title']?['runs']?[0]?['text'] ??
                      videoRenderer['title']?['simpleText'] ?? 'Unknown Title';

        final thumbnails = videoRenderer['thumbnail']?['thumbnails'];
        final thumbUrl = thumbnails != null && thumbnails.isNotEmpty ? thumbnails.last['url'] : '';
  ...
  static String getOptimizedThumbnail(String originalUrl, bool turboMode) {
    ...
    if (originalUrl.contains('hqdefault.jpg')) return originalUrl.replaceFirst('hqdefault.jpg', 'mqdefault.jpg');
    if (originalUrl.contains('sddefault.jpg')) return originalUrl.replaceFirst('sddefault.jpg', 'mqdefault.jpg');
    if (originalUrl.contains('maxresdefault.jpg')) return originalUrl.replaceFirst('maxresdefault.jpg', 'mqdefault.jpg');
