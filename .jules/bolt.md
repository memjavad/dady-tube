...
## 2024-05-24 - [Optimize AnimatedBuilder Child Usage]
**Learning:** In continuous background animations like `ParticleBackground`, capturing the widget subtree directly in the builder function forces 60fps evaluation of potentially complex screens (e.g., `ChannelFeedScreen`), creating severe CPU/memory overhead.
**Action:** Always extract static widget subtrees into the `child` parameter of `AnimatedBuilder` and `TweenAnimationBuilder` to bypass pointless re-renders.
