...
## 2024-04-27 - [Optimize Sequential File Writing]
**Learning:** In Dart, using an `await for` loop with `RandomAccessFile.writeFrom` to read and write streams sequentially creates a significant performance bottleneck due to async event-loop overhead for each chunk.
**Action:** Always use `IOSink.addStream` (via `file.openWrite()`) when writing streams sequentially to a file. It delegates buffering and writing to Dart's optimized internal implementation, significantly reducing overhead.
